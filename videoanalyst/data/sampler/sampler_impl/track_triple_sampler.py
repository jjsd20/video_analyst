...
class TrackTripleSampler(SamplerBase):
    ...
    def __getitem__(self, item) -> dict:
        ...
        data1-template
        data2-search
        data3-update (if is_negative_pair=true ,data3=data1 else,data3=data1+1)
        is_negative_pair = self._sample_track_pair()
    ...
    def _sample_track_pair(self) -> Tuple[Dict, Dict]:
        ...
        if len_seq == 1 and not isinstance(sequence_data["anno"][0], list):
            ...
            # video dataset
            data1, data2, data3 = self._sample_track_pair_from_sequence(
    ...
    def _sample_track_pair_from_sequence(self, sequence_data: Dict,
        ...
        """
        len_seq = self._get_len_seq(sequence_data)
        idx1, idx2, idx3 = self._sample_pair_idx_pair_within_max_diff(
            len_seq, max_diff)
        data1 = {k: v[idx1] for k, v in sequence_data.items()}
        data2 = {k: v[idx2] for k, v in sequence_data.items()}
        data3 = {k: v[idx3] for k, v in sequence_data.items()}
        if isinstance(data1["anno"],
            ...
            data3["anno"] = self._generate_mask_for_vos(data3["anno"])
        return data1, data2, data3
