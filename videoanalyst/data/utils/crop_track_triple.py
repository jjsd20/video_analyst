...
def crop_track_triple(
    ...
    wc_prev, hc_prev = box_prev[2:]
    wc_prev_ = wc_prev + context_amount * (wc_prev + hc_prev)
    hc_prev_ = hc_prev + context_amount * (wc_prev + hc_prev)
    sc_prev_ = np.sqrt(wc_prev_ * hc_prev_)

    assert (st_ > 0) and (
        sc_ > 0), "Invalid box: box_temp %s and box_curr %s and box_prev %s" % (
    ...
    return im_z, bbox_z, im_x, bbox_x, im_prev, bbox_prev, mask_z, mask_x, mask_prev
